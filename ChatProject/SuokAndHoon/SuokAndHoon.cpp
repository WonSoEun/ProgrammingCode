...
{
public:
	void(Hoon::* RunPointer)(Actor* actor); //함수포인터 선언, 매개변수로 Actor 선언 
	...
	void Start(Actor* actor) //Start 함수 오버라이드, 상대 배우의 변수를 받아온다
	{
		...
		th = thread(RunPointer, this, actor); //함수를 지목할 포인터(RunPointer), RunPointer가 가리키고 있는 함수(this = Hoon::Run), 매개변수 actor
	}
